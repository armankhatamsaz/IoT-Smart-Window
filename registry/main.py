...
# قفل سراسری برای جلوگیری از تداخل (Race Condition)
...
DEFAULT_CATALOG = {
    ...
    "usersList": [
        {"userID": 1, "userName": "user1", "password": "1234", "allowed_zones": ["Living Room", "Bedroom 1"]},
        {"userID": 2, "userName": "user2", "password": "1234", "allowed_zones": ["Living Room", "Bedroom 2"]}
    ],
    "servicesList": [],
    "devicesList": []
}
...
class CatalogManager:
    # قفل‌ها رو از اینجا برداشتیم تا تو لایه بالاتر (CherryPy) اعمالشون کنیم
    @staticmethod
    def load():
...
class SmartHomeRegistry(object):
    ...
    def GET(self, *uri, **params):
        ...
        if len(uri) > 0:
            if uri[0] == "broker": return catalog["broker"]
            elif uri[0] == "usersList": return catalog["usersList"]
            elif uri[0] == "devicesList": return catalog["devicesList"]
            elif uri[0] == "servicesList": return catalog["servicesList"]
            elif uri[0] == "config":
                return {"broker": catalog["broker"], "usersList": catalog["usersList"]}
            else: raise cherrypy.HTTPError(404, "Endpoint not found")
        else:
            return catalog 
    ...
    def POST(self, *uri, **params):
        ...
        if not raw_body: raise cherrypy.HTTPError(400, "Empty request body")
        
        new_data = json.loads(raw_body)
        new_data["lastUpdate"] = str(datetime.now().strftime("%Y-%m-%d %H:%M:%S"))
        
        # با قرار دادن قفل اینجا، تا وقتی ثبت یه دستگاه تموم نشده، کسی نمیتونه فایل رو دستکاری کنه
        with file_lock:
            catalog = CatalogManager.load()
            if len(uri) > 0:
                ...
                else: raise cherrypy.HTTPError(404, "Use /devicesList or /servicesList")
            else: raise cherrypy.HTTPError(400, "Missing endpoint")
    ...
    def PUT(self, *uri, **params):
        ...
        with file_lock:
            ...
            if len(uri) > 0:
                ...
                else: raise cherrypy.HTTPError(404, "Entity not found for update")
            else: raise cherrypy.HTTPError(400, "Missing endpoint")
